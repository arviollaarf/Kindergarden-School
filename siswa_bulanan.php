...
$query = "SELECT nilaibulanan.*, sis.nama_siswa
          FROM nilai_bul nilaibulanan join siswa sis
          ON nilaibulanan.noinduk_siswa = sis.noinduk_siswa
          WHERE sis.status_regis = '1'
          AND nilaibulanan.noinduk_siswa = '" . $_GET['id'] . "'";
$sql = mysqli_query($conn, $query);
$counter = 1;

$noinduk = $_GET['id'];
$kelbel = mysqli_query($conn, "SELECT sis.*, kel.*, ta.tahunajar, peg.nama_peg, kls.nama_ruang_kelas
...
WHERE sis.noinduk_siswa= '$noinduk'");
?>
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4"> Nilai Bulanan Siswa</h1>
                    <div class="card mb-4">
                            ...
                            <table id="datatablesSimple">
                                ...
                                <thead>
                                    <tr>
                                        <th>Kode</th>
                                        <th>NIS</th>
                                        <th>Bulan Ambil Nilai</th>
                                        <th width="1%">Detail Nilai</th>
                                    </tr>
                                </thead>
                                <tbody>
                                <?php
                                    $ambil = $conn->query("SELECT * FROM nilai_bulanan");
                                    ?>
                                    <?php while ($row = $ambil->fetch_assoc()) {    ?>
                                        <tr>
                                            <td><?php echo $row['id_nilai_bulanan']; ?></td>
                                            <td><?php echo $row['noinduk_siswa']; ?></td>                                          
                                            <td><?php echo $row['bulan_ambil_nilai']; ?></td>                                           
                                            <td>
                                            <a href="detail-nilai-bulanan.php?id=<?php echo $row['id_nilai_bulanan']; ?>" class="btn btn-warning">Lihat Nilai</a>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                    </tbody>
